...
56. PUN-frame delivery fragility — A player's ability to act must not depend on burst delivery of the `shoot` frame over a single unacknowledged SSE stream: a ~2s stall or a dropped frame (whose recovery, reconnect + snapshot, exceeds the window) turns into "skip PUN → Waiting for result → You lose". Fix (protocol v1.1–v1.3): pre-announce `shootAt` at countdown start (landed in v1.1), add per-frame `ts` (landed), stream seq + replay on reconnect, and a `/ping` health probe — the latter two are parked as symptom descriptions in docs/issues.md until the connectivity diagnostics slice (roadmap) confirms their cause. Residual no-move path: a round resolving with one valid move scores the no-move side as `void` (no-contest, like a draw) instead of `loss`, so connectivity never reads as a streak-breaking loss (scheduled; see the roadmap).
